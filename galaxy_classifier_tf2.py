...
def CNN(img_channels, img_rows, img_cols, verbose=False):
    ...
    # some hyper parameters you can change
    dropoutpar = 0.5
    nb_dense = 64

    model = Sequential()
    model.add(Convolution2D(32, 6, 6, padding='same',
...
npred = int(D.shape[0] - (ntrain + nval))  # test sample size;
...
if my_file.is_file():
    ...
    print("====================")
    model_name = train_CNN(D, Y, int(ntrain), int(nval), output=save_files)
    Y_pred = apply_CNN(D[pred_index:pred_index + npred, :, :, :], save_files)
    Y_test = Y[pred_index:pred_index + npred]
